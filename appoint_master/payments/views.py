    ...
    try:
        ...
        if verified:
            ...
            message = (f"Dear Dr. {doctor.first_name.title()} {doctor.last_name.title()},\n\nA new appointment : {appointment.appoint_id} , "
                       ...
                       f"Please make effort to be present on time. Your appointment starts in {(adjusted_appoint_time - (timezone.now() + timedelta(hours=3))).days} days.\n\n "
                       f" Video Call URL : {str(settings.INITIAL_MEET_KEY) + str(appointment.appoint_id)} \n\n"
                       "Sanjeevni Ai | All Rights reserved.")
            ...
            message = (f"Hello, {patient.first_name.title()} {patient.last_name.title()}! Your appointment {appointment.appoint_id} with doctor "
                       ...
                       f"{adjusted_appoint_time.strftime('%I:%M %p, %d %B %Y')}. \n\n Please make effort to be present on that day. Your appointment starts in "
                       f"{(adjusted_appoint_time - (timezone.now() + timedelta(hours=3))).days} days. \n\n"
                       f" Video Call URL : {settings.INITIAL_MEET_KEY + appointment.appoint_id} \n\n"
                       "Sanjeevni Ai | All Rights reserved.")
            ...
            if payment.appointment:
                ...
                message = (f"Hello, {patient.first_name.title()} {patient.last_name.title()}! Your appointment {appointment.appoint_id} "
                           "has failed. Please try again. Sanjeevni Ai | All Rights reserved.")
                send_mail(subject, message, email_from, [patient.email])
        ...
        messages.error(request, f"error : {e}, Oops! An error occurred while verifying the payment. Please try again later.")
