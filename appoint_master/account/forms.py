...
from .models import ID, User, Appointment, generate_appointment_id
...
class DoctorRegistrationForm(UserCreationForm):
    ...
    state_name = forms.ChoiceField(required=True, label='STATE_Name_CHOICES', choices=STATE_Name_CHOICES, widget=forms.Select(attrs={'id': 'STATE_Name_CHOICES', 'required': True, 'class': 'block w-full rounded-full border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6'}))
    email = forms.EmailField(required=True, widget=forms.EmailInput(attrs={'class': 'block w-full rounded-full border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6', 'id': 'email', 'autocomplete': 'email', 'placeholder': 'Email'}))
    username = forms.CharField(label='Username', widget=forms.TextInput(attrs={'id': 'username', 'placeholder': 'Unique username', 'required': True, 'class': 'block w-full rounded-full border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6'}), required=True)
    ...
    class Meta:
        model = User
        fields = ['username', 'specialization','state_name', 'first_name', 'last_name',  'email', 'password1', 'password2']
...
class PatientRegistrationForm(UserCreationForm):
    ...
    patient_id = forms.CharField(label='Patient ID', required=True, widget=forms.TextInput(attrs={'id': 'patient_id', 'autocomplete': '', 'placeholder': 'Enter your ID(aadhar,pan,etc)', 'required': True, 'class': 'block w-full rounded-full border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6'}), max_length=15)
    state_name = forms.ChoiceField(required=True, label='STATE_Name_CHOICES', choices=STATE_Name_CHOICES, widget=forms.Select(attrs={'id': 'STATE_Name_CHOICES', 'required': True, 'class': 'block w-full rounded-full border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6'}))
    ...
    class Meta:
        model = User
        fields = ['username', 'patient_type','state_name',  'patient_id', 'first_name', 'last_name', 'email', 'phone_number',  'password1', 'password2', 'agree_to_terms']
    ...
    def clean_patient_id(self):
        ...
        try:
            ...
            pass  # No matching record found, so it's fine

        except MultipleObjectsReturned:
            raise forms.ValidationError("Invalid user ID (Multiple records found)")
        return patient_id
...
class PatientRegistrationForm_Admin(UserCreationForm):
    ...
    class Meta:
        model = User
        fields = ['username', 'patient_type', 'state_name', 'patient_id', 'first_name', 'last_name', 'email',  'password1', 'password2']
...
class RestrictedDateTimeField(forms.DateTimeField):
    def validate(self, value):
        ...
        if value:
            ...
            if value.time() < time(8, 0) or value.time() > time(22, 0):  # Check if time is outside 8am-10pm
                raise ValidationError("Appointment time should be between 8 am and 10 pm.")
            ...
            if value.minute % 10 != 0:  
                raise ValidationError("Appointment time should be in multiples of 10 minutes (00, 10, 20, 30, 40, etc).")
...
class UpdateProfileForm_patient(forms.ModelForm):   
    ...
    class Meta:
        model = User
        fields = ['profile_photo','username', 'first_name', 'last_name', 'gender','state_name', 'email', 'phone_number']
