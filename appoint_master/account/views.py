...
from .forms import AdminRegistrationForm, CreateAppointmentForm_patient, DoctorRegistrationForm, PatientRegistrationForm_Admin, PatientRegistrationForm, UpdateProfileForm_admin, UpdateProfileForm_doctor, UpdateProfileForm_patient
from payments.models import Payment
from .models import ID, Appointment, User
        ...
        if user is not None:
            if user.active:
                ...
                try:
                    ...
                    messages.error(request, f"Oops! An error occurred. Check your internet connection. or {e}")
                    # Oops! An error occurred. Check your internet connection.
                    # Header values can't contain newlines (got 'OTP Verification Code from Sanjeevni Ai \n\n ' for header 'Subject')
...
def send_mail_otp(email, otp):
    try:
        subject = 'OTP Verification Code from Sanjeevni Ai'
        message = f'Your OTP for login is: {otp} from Sanjeevni Ai'
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                subject = 'Sanjeevni Ai'
                email_from = settings.EMAIL_HOST_USER
                message = f'Your account has been permanently deleted!\nWe are sad to see you leave Sanjeevni Ai!\n\n.'
                ...
                if doctor:
                    ...
                    try:
                        ...
                        message = (f"Hello, {patient.first_name.title()} {patient.last_name.title()}! Your appointment request "
                                   ...
                                   "Sanjeevni Ai Sanjeevni Ai©2025 | All Rights reserved.")
...
# Create a view to show specific patient appointment 
...
def view_appointments_manage(request):
    ...
    patient_type = "Student" if user.is_Student_Patient else "Staff" if user.is_Staff_Patient else "None"
    meet_url = str(settings.INITIAL_MEET_KEY) 
    ...
    return render(request, 'appoint_app/view_appointments_manage.html', {'all_user_appointments': all_user_appointments, 'patient_type':patient_type, 'meet_url':meet_url})
...
def all_appointments_doctor(request):
    ...
    return render(request, 'appoint_app/view_appointments_list_doctor.html', {'appointments_list': appointments_list,'meet_url':meet_url})
        ...
        try:
            ...
            try:
                ...
                message = (f"Hello, {patient.first_name.title()} {patient.last_name.title()}! Your appointment {appointment.appoint_id} with doctor "
                           f"{doctor.first_name.title()} {doctor.last_name.title()} has been cancelled. Sanjeevni Ai™ Sanjeevni Ai©2025 | All Rights reserved.")
                send_mail(subject, message, email_from, [patient.email])
            except Exception as e:
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                email_from = settings.EMAIL_HOST_USER
                message = (f"Hello, {patient.first_name.title()} {patient.last_name.title()}! Your appointment {appointment.appoint_id} with doctor "
                           f"{doctor.first_name.title()} {doctor.last_name.title()} has been completed. Sanjeevni Ai™ Sanjeevni Ai©2025 | All Rights reserved.")
                send_mail(subject, message, email_from, [patient.email])
            except Exception as e:
...
def video_call(request,appointment_id):
    ...
    meet_url = settings.INITIAL_MEET_KEY + appointment_id
    INITIAL_MEET_KEY = settings.INITIAL_MEET_KEY
    JISTY_KEY = settings.JISTY_KEY
    JISTY_JWT_KEY = settings.JISTY_JWT_KEY
    appoint_area = appointment.appoint_area
    ...
    context = {
        ...
        'doctor':doctor,
        'meet_url':meet_url,
        'INITIAL_MEET_KEY':INITIAL_MEET_KEY,
        'JISTY_KEY':JISTY_KEY,
        'JISTY_JWT_KEY':JISTY_JWT_KEY,
        'appoint_area':appoint_area
    }
  
    return render(request, "appoint_app/video_call.html",context)


# Create a view for video calls (for Patients)     
def video_call_patient(request,appointment_id):
    appointment = get_object_or_404(Appointment, appoint_id=appointment_id)
    patient = appointment.appointee
    doctor = appointment.appointed_doctor
    meet_url = settings.INITIAL_MEET_KEY + appointment_id
    ...
    return render(request, "appoint_app/video_call_patient.html",context)      
...
# Only the Particular doctor can write the specific prescription only at onces.
# @login_required
# def add_or_edit_prescription(request, appoint_id):
#     appointment = get_object_or_404(Appointment, appoint_id=appoint_id)
    
#     if request.user.username != appointment.appointed_doctor.username :
#         return HttpResponseForbidden("You are not authorized to add a prescription for this appointment.\n Only the specific doctor can create/edit their appointed prescription.")
    
#     prescription = Prescription.objects.filter(appointment_id=appoint_id).first()
#     # print("prescription : " ,prescription)
#     # print(prescription.content)
#     if prescription:
#         form = PrescriptionForm(instance=prescription)  # Pre-fill form for editing
#     else:
#         form = PrescriptionForm()  # Empty form for creating

#     if request.method == "POST":
#         if prescription:
#             form = PrescriptionForm(request.POST, instance=prescription)  # Update existing prescription
#         else:
#             form = PrescriptionForm(request.POST)  # Create new prescription
        
#         if form.is_valid():
#             prescription = form.save(commit=False)
#             prescription.appointment_id = appoint_id
#             prescription.doctor = request.user  # Assigning logged-in doctor
#             prescription.save()

#             return redirect('view_prescription', appoint_id=appointment.appoint_id)
#     # context= {'form': form, 'appointment': appointment,'prescription_content':prescription.content}
#     context= {'form': form, 'appointment': appointment}
#     return render(request, 'appoint_app/prescription/add_prescription.html',context)
def add_or_edit_prescription(request, appoint_id):
    appointment = get_object_or_404(Appointment, appoint_id=appoint_id)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            prescription.prescribed_by = request.user  # Assign logged-in doctor
            prescription.prescribed_for_patient = appointment.appointee  # Assign the patient
            prescription.save()
...
# Ensure only the Particular patient & doctor can view their specific prescription.
@login_required
def view_prescription(request, appoint_id):
    appointment = get_object_or_404(Appointment, appoint_id=appoint_id)

    if request.user.username != appointment.appointee.username and request.user.username != appointment.appointed_doctor.username  :
        return HttpResponseForbidden("You are not authorized to view this prescription.\n Only the specific patient and doctor can view their appointed prescription. ")

    return render(request, 'appoint_app/prescription/view_prescription.html', {'prescription': appointment.prescription})

def list_prescription(request):
    user = request.user  
    try:
        if user.is_Doctor : 
            all_prescriptions = Prescription.objects.filter(appointment__appointed_doctor=user)
            all_completed_scheduled_appoint = Appointment.objects.filter(appointed_doctor=user, appoint_status__in=['Completed', 'Scheduled']  )
            return render(request, 'appoint_app/prescription/list_prescription.html',{'all_prescriptions': all_prescriptions,'all_completed_scheduled_appoint':all_completed_scheduled_appoint})

        elif user.is_Student_Patient or user.is_Staff_Patient  :
            all_prescriptions = Prescription.objects.filter(appointment__appointee=user)
            return render(request, 'appoint_app/prescription/list_prescription.html',{'all_prescriptions': all_prescriptions})

        else: #admin
            all_prescriptions = Prescription.objects.all()
            all_completed_scheduled_appoint = Prescription.objects.filter(appointment__appointed_doctor=user, appoint_status__in=['Completed', 'Scheduled']  )
            return render(request, 'appoint_app/prescription/list_prescription.html',{'all_prescriptions': all_prescriptions,'all_completed_scheduled_appoint':all_completed_scheduled_appoint})
        
    except Exception as e:
        return HttpResponseServerError("An error occurred while fetching prescriptions: " + str(e))  
