...
from .models import ID, Appointment, Prescription, User
...
class CustomUserAdmin(UserAdmin):
    ...
    list_filter = ('is_Student_Patient', 'is_Staff_Patient', 'is_Doctor', 'is_staff', 'username', 'last_login', 'last_logout', 'first_name', 'last_name', 'email', 'patient_id', 'gender', 'phone_number', 'specialization','date_joined', 'active',) # 'next_of_kin','has_next_of_kin',
...
class PrescriptionAdmin(admin.ModelAdmin):
    list_display = ('appointment', 'prescribed_by', 'created_at', 'updated_at')  
    list_filter = ('prescribed_by', 'created_at', 'updated_at')  
    search_fields = ('appointment__appoint_id', 'prescribed_by__username', 'content')
    readonly_fields = ('created_at', 'updated_at')  

    def get_queryset(self, request):
        """Optimize queries for better performance."""
        return super().get_queryset(request).select_related('appointment', 'prescribed_by')